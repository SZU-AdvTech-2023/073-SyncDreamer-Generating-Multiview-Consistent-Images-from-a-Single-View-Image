...
from ldm.ldm_utils import instantiate_from_config
...
class SpatialVolumeNet(nn.Module):
    def __init__(self, time_dim, view_dim, view_num,
        ...
        super().__init__()
        print("time_dim: ", time_dim)       # 256
        print("view_dim: ", view_dim)       # 4
        print("view_num: ", view_num)       # 16
...
class SyncMultiviewDiffusion(pl.LightningModule):
    ...
    def get_target_view_feats(self, x_input, spatial_volume, clip_embed, t_embed, v_embed, target_index):
        ...
        frustum_volume_feats, frustum_volume_depth = self.spatial_volume.construct_view_frustum_volume(spatial_volume,
                                                                                                       t_embed, v_embed,
                                                                                                       self.poses,
                                                                                                       self.Ks,
                                                                                                       target_index)
    ...
    def training_step(self, batch):
        ...
        # (4, 4, 32, 32)
        # print("spatial_volume shape: ", spatial_volume.shape)
...
class SyncDDIMSampler:
    ...
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale,
                      batch_view_num=1, is_step0=False):
        """
        @param x_target_noisy:   B,N,4,H,W
        @param input_info:
        @param clip_embed:       B,M,768
        ...
        # print("x_target_noisy shape", x_target_noisy.shape)
        B, N, C, H, W = x_target_noisy.shape        # 4, 16, 4, 32, 32
        ...
        spatial_volume = self.model.spatial_volume.construct_spatial_volume(x_target_noisy, t_embed, v_embed,
        ...
        for ni in range(0, N, batch_view_num):
            ...
            clip_embed_, volume_feats_, x_concat_ = self.model.get_target_view_feats(x_input, spatial_volume,
                                                                                     clip_embed, t_embed, v_embed,
                                                                                     target_indices_)
            if unconditional_scale != 1.0:
                noise = self.model.model.predict_with_unconditional_scale(x_target_noisy_, time_steps_, clip_embed_,
                                                                          volume_feats_, x_concat_, unconditional_scale)
            else:
                noise = self.model.model(x_target_noisy_, time_steps_, clip_embed_, volume_feats_, x_concat_,
                                         is_train=False)
    ...
    def sample(self, input_info, clip_embed, unconditional_scale=1.0, log_every_t=50, batch_view_num=1):
        ...
        for i, step in enumerate(iterator):
            ...
            x_target_noisy = self.denoise_apply(x_target_noisy, input_info, clip_embed, time_steps, index,
                                                unconditional_scale, batch_view_num=batch_view_num, is_step0=index == 0)
